...
class NSU_Form {
	...
	public function output_form( $echo = true ) {
		...
		if ( ! isset( $_POST['nsu_submit'] ) || count( $errors ) > 0 ) {
			...
			$output .= "<label for=\"nsu-email-$formno\">$email_label</label><input class=\"nsu-field\" id=\"nsu-email-$formno\" type=\"$email_type\" name=\"$email_id\" $email_atts ";
			$output .= "/>";
			...
			$output .= sprintf( '<input type="submit" id="nsu-submit-%s" class="nsu-submit" name="nsu_submit" value="%s" />', $formno, esc_attr( $submit_button ) );
